...
describe("withNormalizedErrors", () => {
    ...
    // @capacitor/core's registerPlugin proxy returns a new function on every
    // property read.
    it("keeps one wrapper per method when the plugin hands out a fresh function per read", async () => {
        const target = new Proxy({} as Record<string, () => Promise<string>>, {
            get: (_object, property) => () => Promise.resolve(String(property)),
        });
        const plugin = withNormalizedErrors(target);

        expect(plugin.getOfferings).toBe(plugin.getOfferings);
        await expect(plugin.getOfferings()).resolves.toBe("getOfferings");
    });
